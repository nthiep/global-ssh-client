...
def conninfo(user, peer, hashcode):
    for u in sv_lsclient.lsclient:
        if u.name == user:
            res = [{"status": 200, "hashcode" : hashcode, "peer" : peer, "address" : u.addr, "localadd" : u.localadd}]
...
def connect(user, peer):
    ...
    ju = conninfo(user, peer, hashcode)
    jp = conninfo(peer, user, hashcode)
    for p in sv_lsclient.lsclient:
        if p.name == peer:
            p.connection.send(ju)
            u.addlogs(str(datetime.datetime.now()), 'connecting to %s(%s) at %s' % (peer, p.addr, request.remote_addr))
            f.addlogs(str(datetime.datetime.now()), 'connecting from %s(%s) at %s' % (user, request.remote_addr, p.addr))  
    return jp
def upload(user, key):
    u = User(user)
    u.uploadkey(key)
